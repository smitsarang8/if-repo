...
const favourite = ({ }) => {
	...
	useEffect(() => {
		...
		if (pType === 'all' && area === "all") {
			address = `https://api.airtable.com/v0/appgsdBi4Ssk6GHRs/listing_requests?filterByFormula=AND({status}='approved',{purpose}='rent',{city}='surat')`;
		} else if (pType != 'all' && area === "all") {
			address = `https://api.airtable.com/v0/appgsdBi4Ssk6GHRs/listing_requests?filterByFormula=AND({status}='approved',{propertyType}='${pType}',{purpose}='rent',{city}='surat')`;
		}
		else if (pType != 'all' && area != "all") {
			address = `https://api.airtable.com/v0/appgsdBi4Ssk6GHRs/listing_requests?filterByFormula=AND({status}='approved',{propertyType}='${pType}',{purpose}='rent',{area}='${area}',{city}='surat')`;
		} else if (pType == 'all' && area != "all") {
			address = `https://api.airtable.com/v0/appgsdBi4Ssk6GHRs/listing_requests?filterByFormula=AND({status}='approved',{purpose}='rent',{area}='${area}',{city}='surat')`;
		}
		fetch(address, {
			method: 'get',
			headers: new Headers({
	...
	const address = "https://api.airtable.com/v0/appgsdBi4Ssk6GHRs/listing_requests?filterByFormula=AND({status}='approved',{purpose}='rent')";
				...
				<div class="px-4 max-w-2xl mx-auto py-12 sm:py-12 lg:py-12 lg:max-w-none">
					<div class="lg:w-1/2 w-full py-6 lg:mb-0">
						<h1 class="px-4 sm:text-2xl text-xl sm-title text-gray-900">
							Properties available for rent in Surat
						</h1>
