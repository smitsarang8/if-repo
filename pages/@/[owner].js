...
const favourite = ({ authors,context,products }) => {
	...
	const loadProperties = () => {
		...
		products.records.map((p, index) => {
			if (p.fields.owner_slug == router.query.owner)
				// console.log(index)
				whichOne.push(index)
				console.log(...new Set(whichOne))
		})
	...
	const loadUserInfo = () => {
		...
		authors.records.map((p, index) => {
			if (p.fields.slug == router.query.owner)
				// console.log(index)
				whichOne.push(index)
		})
		const amenitiesArray = "adf,afd,f,dasdsa".split(",")
				...
				records: [
					{
						fields: {
							...
							business_slug: author.records[0].fields.slug,
							status: "pending",
							timestamp: datetime
						}
					}
	...
	const handleShareButton = () => {
		...
		if (navigator.share) {
			navigator
				.share({
					url: "https://rdtesting.netlify.app/" + authors.records[0].fields.slug
				})
				.then(() => {
				})
				.catch(() => {
			...
			navigator.clipboard.writeText("https://rdtesting.netlify.app/" + authors.records[0].fields.slug)
			setClipboard(true)
		}
	};
	return (
		<><Head>
			<title>a- Real Dukaan</title>
