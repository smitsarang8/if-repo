...
const shop = ({ product, email }) => {
	...
	const filterShops = async event => {
		event.preventDefault();
		// Router.push("/shops/" + event.target.purpose.value + "-" + event.target.city.value + "-" + event.target.area.value)
				...
				<div class="px-4 max-w-2xl mx-auto py-12 sm:py-12 lg:py-12 lg:max-w-none">
					...
					<div class="lg:w-1/2 w-full py-6 lg:mb-0">
								...
								>
									<div class="relative px-4 w-full max-w-2xl h-full md:h-auto">
										<div class="relative bg-white rounded-lg shadow ">
											...
											<div class="p-6 ">
												<form onSubmit={filterShops} class="" method="POST">
						...
						<h1 class="px-4 sm:text-3xl text-2xl sm-title text-gray-900">
							Shops available for rent
						</h1>
						<br/>						
						<p class="px-4 text-gray-800">Suggestions: <a href="/shops/buy" class="ml-2 py-2 px-2 rounded-full text-indigo-500 border border-indigo-500">Buy shops</a></p>
						<br/>						
...
export const getStaticProps = async (context) => {
	const ress = await fetch("https://api.airtable.com/v0/appgsdBi4Ssk6GHRs/listing_requests?&filterByFormula=AND({status}='approved',{propertyType}='shop',{purpose}='rent')", {
		method: 'get',
		headers: new Headers({
			'Authorization': 'Bearer keyLRae2Fru3dnFqr',
		}),
